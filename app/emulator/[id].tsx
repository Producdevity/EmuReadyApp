...
export default function EmulatorDetailScreen() {
  ...
  const handleShare = async () => {
    try {
      const shareContent = `Check out ${emulatorQuery.data?.name} on EmuReady!\n\n` +
          ...
          <View style={styles.errorContainer}>
            <Animated.View entering={FadeInUp.delay(200).springify()}>
              <Card variant="glass" padding="lg" style={{ alignItems: 'center' }}>
                ...
                />
                <Text style={[styles.errorTitle, { color: theme.colors.text }]}>Emulator Not Found</Text>
          ...
          <Card variant="glass" padding="sm">
            <View style={styles.tabButtons}>
              <Pressable
                style={[
                  styles.tab,
                  { backgroundColor: selectedTab === 'overview' ? theme.colors.primary : 'transparent' }
                ]}
              ...
              <Pressable
                style={[
                  styles.tab,
                  { backgroundColor: selectedTab === 'listings' ? theme.colors.primary : 'transparent' }
                ]}
          ...
          <View style={styles.tabContent}>
            ...
            <Animated.View entering={FadeInUp.delay(800).springify()}>
              <Card variant="glass" style={styles.infoCard} padding="lg">
                <Text style={[styles.sectionTitle, { color: theme.colors.text }]}>Emulator Information</Text>
                <View style={styles.infoRow}>
                  <Text style={[styles.infoLabel, { color: theme.colors.textMuted }]}>Name:</Text>
                  <Text style={[styles.infoValue, { color: theme.colors.text }]}>{emulatorQuery.data.name}</Text>
                </View>
                <View style={styles.infoRow}>
                  <Text style={[styles.infoLabel, { color: theme.colors.textMuted }]}>Supported Systems:</Text>
                ...
                {emulatorQuery.data.description && (
                  <View style={[styles.infoRow, { flexDirection: 'column', alignItems: 'flex-start' }]}>
                    <Text style={[styles.infoLabel, { color: theme.colors.textMuted, marginBottom: 8 }]}>Description:</Text>
                    <Text style={[styles.infoValue, { color: theme.colors.text }]}>{emulatorQuery.data.description}</Text>
                  </View>
                )}
              </Card>
            </Animated.View>
                  ...
                  <View style={styles.performanceGrid}>
                    {getPerformanceStats(listings).map((stat, index) => (
                      <View key={index} style={styles.performanceItem}>
                        ...
                        />
                        <Text style={[styles.performanceLabel, { color: theme.colors.text }]}>{stat.label}</Text>
                        <Text style={[styles.performanceCount, { color: theme.colors.textMuted }]}>{stat.count}</Text>
                      </View>
                    ))}
                  </View>
                </Card>
            ...
            {listingsQuery.isLoading ? (
              ...
              <Animated.View entering={FadeInUp.delay(800).springify()}>
                <Card variant="glass" style={styles.emptyCard} padding="lg">
                  ...
                  />
                  <Text style={[styles.emptyTitle, { color: theme.colors.text }]}>No Listings Yet</Text>
