    ...
    return (
      <SafeAreaView style={{ flex: 1, backgroundColor: theme.colors.background }}>
        <View className="flex-1 justify-center items-center px-8">
          ...
          <Button title="Back to GPUs" onPress={() => router.back()} variant="outline" />
        </View>
      </SafeAreaView>
    )
  }
        ...
        <View className="p-4 space-y-4">
          ...
          <Card>
            <View className="p-4">
              ...
              {renderSpecItem('Architecture', gpuQueryResult.data.architecture, 'construct')}
              {renderSpecItem(
                'Process Node',
                gpuQueryResult.data.processNode ? `${gpuQueryResult.data.processNode} nm` : null,
                'resize',
              )}
              {renderSpecItem('Stream Processors', gpuQueryResult.data.streamProcessors, 'grid')}
              {renderSpecItem('RT Cores', gpuQueryResult.data.rtCores, 'cube')}
              {renderSpecItem('Tensor Cores', gpuQueryResult.data.tensorCores, 'cube')}
              {renderSpecItem('Memory Type', gpuQueryResult.data.memoryType, 'hardware-chip')}
              {renderSpecItem(
                'Memory Bus',
                gpuQueryResult.data.memoryBus ? `${gpuQueryResult.data.memoryBus}-bit` : null,
                'swap-horizontal',
              )}
              {renderSpecItem(
                'Memory Bandwidth',
              ...
              {renderSpecItem('PCIe Interface', gpuQueryResult.data.pcieInterface, 'git-network')}
            </View>
          </Card>
          ...
          <Card>
            <View className="p-4">
              ...
              {renderSpecItem('Max Resolution', gpuQueryResult.data.maxResolution, 'desktop')}
              {renderSpecItem('Max Displays', gpuQueryResult.data.maxDisplays, 'tv')}
              {renderSpecItem('DisplayPort', gpuQueryResult.data.displayPorts, 'tv')}
              {renderSpecItem('HDMI Ports', gpuQueryResult.data.hdmiPorts, 'tv')}
              {renderSpecItem('USB-C Ports', gpuQueryResult.data.usbcPorts, 'cable')}
            </View>
          </Card>
            ...
            <Card>
              <View className="p-4">
                ...
                {gpuQueryResult.data.releaseDate &&
                  renderSpecItem(
                    'Release Date',
                    new Date(gpuQueryResult.data.releaseDate).toLocaleDateString(),
                    'calendar',
                  )}
                {gpuQueryResult.data.releaseYear &&
                  renderSpecItem('Release Year', gpuQueryResult.data.releaseYear, 'calendar')}
                {gpuQueryResult.data.msrp &&
                  renderSpecItem('MSRP', `$${gpuQueryResult.data.msrp}`, 'pricetag')}
                {gpuQueryResult.data.codename &&
                  renderSpecItem('Codename', gpuQueryResult.data.codename, 'code')}
                {gpuQueryResult.data.foundryName &&
                  renderSpecItem('Foundry', gpuQueryResult.data.foundryName, 'business')}
              </View>
            </Card>
          )}

          {/* Features */}
          {gpuQueryResult.data.features && gpuQueryResult.data.features.length > 0 && (
            <Card>
              <View className="p-4">
                <Text className="text-lg font-semibold mb-3" style={{ color: theme.colors.text }}>
                  Features
                ...
                <View className="flex-row flex-wrap gap-2">
                  {gpuQueryResult.data.features.map((feature: string, index: number) => (
                    <View
                      key={index}
          ...
          <Card>
            <View className="p-4">
              <View className="flex-row justify-between items-center mb-3">
                ...
                <Button
                  title="View All"
                  onPress={() => router.push(`/pc?gpuId=${gpuQueryResult.data.id}`)}
                  variant="outline"
                  size="sm"
                />
              </View>
              ...
              <Text style={{ color: theme.colors.textSecondary }}>
                See how this GPU performs in PC gaming across different games and configurations.
              </Text>
            </View>
          </Card>
        </View>
