...
export default function CPUsScreen() {
  ...
  const handleCpuPress = (id: string) => {
    router.push(`/hardware/cpus/${id}`)
  }

  const renderCpuItem = ({ item }: { item: any }) => (
    <Card style={{ marginBottom: theme.spacing.md, marginHorizontal: theme.spacing.md }}>
      <View className="p-4">
        <View className="flex-row justify-between items-start mb-3">
          ...
          <Button
            title="Details"
            onPress={() => handleCpuPress(item.id)}
            variant="outline"
            size="sm"
          />
        </View>
  ...
  const renderEmptyState = () => (
    <View className="flex-1 justify-center items-center px-8">
      ...
      <Text className="text-center mb-6" style={{ color: theme.colors.textSecondary }}>
        Try adjusting your search or filter criteria to find CPUs.
      </Text>
    </View>
  )
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.colors.background }}>
      ...
      <View className="px-4 py-3 border-b" style={{ borderBottomColor: theme.colors.border }}>
        ...
        <View className="relative">
          <TextInput
            placeholder="Search CPUs by name, model, or series..."
            value={search}
